...
struct ini{
  ...
  void set(){
	...
	if((flag=!inFile.fail())){
      ...
      {
        char * icemodel = getenv("ICEM");
  	ifstream inFile((dir+icemodel).c_str(), ifstream::in);
	if(!inFile.fail()){
	  size=0;
	  ...
	  inFile.close();
	  if(size<2){ cerr << "File "<<icemodel<<" found, but is corrupt" << endl; exit(1); }
	}
	else{ cerr << "File "<<icemodel<<" was not found" << endl; exit(1); }
      }
