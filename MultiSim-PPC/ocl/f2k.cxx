...
template <class T> void addp(float rx, float ry, float rz, float t, float E, T xt){
  ...
  unsigned long long num=poidev(yield(E, xt)/ovr);
...
void addp(float rx, float ry, float rz, float t, unsigned long long n, float dr, float beta){
  p.type=0;
  p.r.w=t; p.r.x=rx; p.r.y=ry; p.r.z=rz;
...
void addp(float rx, float ry, float rz, float t, float light, float length, float beta, float tau, float fr){
  p.type=0;
  p.r.w=t; p.r.x=rx; p.r.y=ry; p.r.z=rz;
